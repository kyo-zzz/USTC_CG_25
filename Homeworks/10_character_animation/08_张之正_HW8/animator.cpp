...
void Joint::compute_world_transform()
{
    ...
    for(int i=0;i<children_.size();i++){
        children_[i]->world_transform_=children_[i]->local_transform_*world_transform_;
        children_[i]->compute_world_transform();
    }
    // --------------------------------------------------------------------------------
}
...
void Animator::update_mesh_vertices()
{
    ...
    int n=indices.size()/vertices.size();
    for(int i=0;i<vertices.size();i++){
	    GfVec3f x={0,0,0};
        for(int j=0;j<n;j++){
            auto joint=joint_tree_.get_joint(indices[i*n+j]);
            x+=weight[i*n+j]*joint->get_world_transform().TransformAffine(joint->get_bind_transform().GetInverse().TransformAffine(vertices[i]));
        }
        vertices[i]=x;
    }
