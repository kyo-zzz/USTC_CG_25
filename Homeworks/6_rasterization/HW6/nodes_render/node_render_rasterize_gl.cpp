...
NODE_DEF_OPEN_SCOPE
    ...
    NODE_EXECUTION_FUNCTION(rasterize_impl) {
        ...
        auto tex_coords_texture = resource_allocator.create(texture_desc);
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, position_texture->texture_id, 0);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, depth_texture->texture_id, 0);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_TEXTURE_2D, tex_coords_texture->texture_id, 0);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_2D, diffuseColor_texture->texture_id, 0);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT4, GL_TEXTURE_2D, metallic_roughness->texture_id, 0);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT5, GL_TEXTURE_2D, normal_texture->texture_id, 0);
        ...
        for(int i = 0; i < meshes.size(); ++i) {
            ...
            glDrawElements(
                GL_TRIANGLES,
                static_cast<int>(mesh->triangulatedIndices.size()) * 3,
                GL_UNSIGNED_INT,
                nullptr
            );
            glBindVertexArray(0);
        ...
        params.set_output("TexCoords", tex_coords_texture);
