...
NODE_DEF_OPEN_SCOPE
    ...
    NODE_EXECUTION_FUNCTION(environment_pass) {
        const auto color = params.get_input<TextureHandle>("Color");

        Hd_USTC_CG_Dome_Light *dome_light = nullptr;
        const auto *free_camera = get_free_camera(params);

        for(int i = 0; i < lights.size(); ++i) {
            if(
                const auto light = lights[i];
                light->GetId() != SdfPath::EmptyPath()
            ) {
        ...
        if(!dome_light) {
            for(int i = 0; i < lights.size(); ++i) {
                if(
                    const auto light = lights[i];
                    HdPrimTypeTokens->domeLight == light->GetLightType()
                ) {
                    dome_light = dynamic_cast<Hd_USTC_CG_Dome_Light*>(light);
                    break;
                }
        ...
        const auto depth = params.get_input<TextureHandle>("Depth");
        const auto size = color->desc.size;
        ...
        const auto shaderPath = params.get_input<std::string>("Shader");
        ...
        const auto shader_error = shader_handle->shader.get_error();
