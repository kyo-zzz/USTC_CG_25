...
// Sample a point on the rectangle light and compute direction, pdf, and radiance
Color Hd_USTC_CG_Rect_Light::Sample(
    ...
    const std::function<float()>& uniform_float)
{
    // Uniformly sample (u, v) in [0,1]^2
    float u = uniform_float();
    float v = uniform_float();

    // Compute sampled position on the rectangle
    GfVec3f pos_sampled = corner0 + u * (corner1 - corner0) + v * (corner2 - corner0);
    sampled_light_pos = pos_sampled;

    // Direction from shading point to light sample
    GfVec3f dir_sampled = pos_sampled - pos;
    float dis = dir_sampled.GetLength();
    ...
    // Rectangle normal
    GfVec3f normal = GfCross(corner1 - corner0, corner2 - corner0).GetNormalized();
    float cos_theta = GfDot(-dir, normal);

    // If not visible, return zero
    if (cos_theta < 0) {
        sample_light_pdf = 0.0f;
        return Color(0);
    }
    ...
    sample_light_pdf = (dis * dis) / (cos_theta * area);

    return irradiance;
}

// Intersect a ray with the rectangle light, return radiance if hit
Color Hd_USTC_CG_Rect_Light::Intersect(const GfRay& ray, float& depth)
{
    // Compute rectangle normal and plane
    GfVec3f normal = GfCross(corner1 - corner0, corner2 - corner0).GetNormalized();
    GfPlane plane(normal, GfDot(normal, corner0));

    double t;
    bool flag;
    if (ray.Intersect(plane, &t, &flag) && t > 0) {
        GfVec3d intersection = ray.GetPoint(t);

        // Check if intersection is inside the rectangle using barycentric coordinates
        GfVec3f edge0 = corner1 - corner0;
        GfVec3f edge1 = corner2 - corner0;
        GfVec3f toIntersection = GfVec3f(intersection) - corner0;

        float dot00 = GfDot(edge0, edge0);
        float dot01 = GfDot(edge0, edge1);
        float dot11 = GfDot(edge1, edge1);
        float dot20 = GfDot(toIntersection, edge0);
        float dot21 = GfDot(toIntersection, edge1);
...
// Initialize rectangle light parameters from scene delegate
    ...
    HdDirtyBits* dirtyBits)
{
    ...
    // Compute rectangle corners in world space
    corner0 = GfVec3f(transform.TransformAffine(GfVec3f(-0.5f * width, -0.5f * height, 0)));
    corner1 = GfVec3f(transform.TransformAffine(GfVec3f(-0.5f * width,  0.5f * height, 0)));
    corner2 = GfVec3f(transform.TransformAffine(GfVec3f( 0.5f * width, -0.5f * height, 0)));
    corner3 = GfVec3f(transform.TransformAffine(GfVec3f( 0.5f * width,  0.5f * height, 0)));

    // Get light color and compute power
    ...
    // Compute irradiance as power divided by area
