...
void SPHBase::compute_density()
{
    ...
    for(size_t i = 0; i < particles.size(); ++i) {
        ...
        for(const auto& q : p->neighbors()) {
            const Vector3d delta_x = p->x() - q->x();
            density += mass * W(delta_x,h);
        }

        // ensure density is not lower than base value
        p->density() = std::max(density,density0);
    }
}
...
void SPHBase::compute_non_pressure_acceleration() {
    const auto& particles = ps_.particles();
    const Vector3d gravity = this->gravity();
    ...
    const std::shared_ptr<Particle>& q)
{
    ...
    double epsilon = 0.01 * ps_.h() * ps_.h();
    Vector3d laplace_v = 2 * viscosity_ * ps_.mass() * v_ij.dot(x_ij) * grad /
        (q->density() * (x_ij.squaredNorm() + epsilon));
...
void SPHBase::compute_pressure_gradient_acceleration() {
    ...
    for(size_t i = 0; i < particles.size(); ++i) {
        auto& p = particles[i];
        // p->acceleration().setZero(); // É¾³ý´ËÐÐ
        const double p_term = p->pressure() / (p->density() * p->density());
        for(const auto& q : p->neighbors()) {
            const Vector3d dx = p->x() - q->x();
            const Vector3d grad = grad_W(dx,h);

            const double q_term = q->pressure() / (q->density() * q->density());

            p->acceleration() -= mass * (p_term + q_term) * grad;
        }
    }
}
